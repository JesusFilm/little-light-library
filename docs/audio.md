...
The prototype uses Kokoro Voice Lab's local Kokoro-82M model to render each authored text segment and each figurine name as a separate 24 kHz mono WAV. Those source recordings are retained under `assets/source-recordings/`. The browser serves local, gapless MP3 derivatives as static assets; it does not call the lab or a speech service. Segment keys follow `locale/story/page/segment`, and names use `locale/names/character`. The runtime `src` and measured duration for each recording are in `public/audio-manifest.json`. See [mobile media derivatives](mobile-media.md) for the conversion workflow.
